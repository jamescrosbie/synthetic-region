...
vanguardsNames = pd.read_csv( path + '/vNameLookup.csv')
vanguards = vanguards.merge(vanguardsNames, on='ID_Org', how='left')

#tidy up
del VanguardDemographics, activity_base, vanguardClusters, vanguards2, vanguards_scaled, vanguardsNames
...
for i in vanguardList:
   ...
   for n in range(nRuns):
        try:
            nonVanguards = np.random.choice(activity['GPCode'].unique().tolist(),size=sample_size,replace=False)
        except:
            sample_size = len(activity['GPCode'].unique().tolist())
            nonVanguards = np.random.choice(activity['GPCode'].unique().tolist(),size=sample_size,replace=False)
...
#
for i in [26,27,28, 52,53 ]:#vanguardList:
    Vname = vanguards['Name'][ vanguards['ID_Org'] == i].iloc[0]
